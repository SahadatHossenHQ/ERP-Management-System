    ...
    <div class="col-sm-8">
        ...
        <?php if (!empty($can_edit) && !empty($edited)) { ?>
            ...
            <a data-toggle="modal" data-target="#myModal_lg"
               href="<?= base_url() ?>admin/estimates/insert_items/<?= $estimates_info->requisition_id ?>"
            ...
            <a class="btn btn-xs btn-success"
               href="<?= base_url() ?>admin/estimates/change_status/hide/<?= $estimates_info->requisition_id ?>"
               title="<?= lang('hide_to_client') ?>"><i class="fa fa-eye-slash"></i> <?= lang('hide_to_client') ?>
                </a><?php } else { ?>
            <a class="btn btn-xs btn-warning"
               href="<?= base_url() ?>admin/estimates/change_status/show/<?= $estimates_info->requisition_id ?>"
        ...
        <?php if (!empty($can_edit) && !empty($edited)) { ?>
            <div class="btn-group">
                ...
                <ul class="dropdown-menu animated zoomIn">
                    <li>
                        <a href="<?= base_url() ?>admin/estimates/index/email_estimates/<?= $estimates_info->requisition_id ?>"
                           data-toggle="ajaxModal"><?= lang('email_estimate') ?></a></li>
                    <li>
                        <a href="<?= base_url() ?>admin/estimates/index/estimates_history/<?= $estimates_info->requisition_id ?>"><?= lang('estimate_history') ?></a>
                    </li>
                    ...
                    <?php if ($estimates_info->status == 'expired' || $estimates_info->status == 'sent' || $estimates_info->status == 'cancelled' || $estimates_info->status == 'draft') { ?>
                        <li>
                            <a href="<?= base_url() ?>admin/estimates/change_status/pending/<?= $estimates_info->requisition_id ?>"
                               title="<?= lang('mark_as_pending') ?>"><?= lang('mark_as_pending') ?></a>
                        </li>
                    <?php } ?>
                    <?php if ($estimates_info->status == 'draft') { ?>
                        <li>
                            <a href="<?= base_url() ?>admin/estimates/change_status/draft/<?= $estimates_info->requisition_id ?>"
                               title="<?= lang('unmark_as_draft') ?>"><?= lang('mark_as_draft') ?></a>
                        </li>
                    <?php } ?>
                    <?php if ($estimates_info->status != 'sent' || $estimates_info->status == 'expired') { ?>
                        <li>
                            <a href="<?= base_url() ?>admin/estimates/change_status/sent/<?= $estimates_info->requisition_id ?>"
                               title="<?= lang('mark_as_sent') ?>"><?= lang('mark_as_sent') ?></a>
                        </li>
                    <?php } ?>
                    <?php if ($estimates_info->status == 'pending' || $estimates_info->status == 'sent') { ?>
                        <li>
                            <a href="<?= base_url() ?>admin/estimates/change_status/expired/<?= $estimates_info->requisition_id ?>"
                               title="<?= lang('mark_as_sent') ?>"><?= lang('mark_as_expired') ?></a>
                        </li>
                    <?php } ?>
                    <?php if ($estimates_info->status != 'cancelled') { ?>
                        <li>
                            <a href="<?= base_url() ?>admin/estimates/change_status/cancelled/<?= $estimates_info->requisition_id ?>"
                    ...
                    <?php } ?>
                    <li>
                        <a href="<?= base_url() ?>admin/estimates/change_status/declined/<?= $estimates_info->requisition_id ?>"><?= lang('declined') ?></a>
                    </li>
                    <li>
                        <a href="<?= base_url() ?>admin/estimates/change_status/accepted/<?= $estimates_info->requisition_id ?>"><?= lang('accepted') ?></a>
                    </li>
                    <?php if (!empty($can_edit) && !empty($edited)) { ?>
                        <li class="divider"></li>
                        <li>
...
if (strtotime($estimates_info->due_date) < strtotime(date('Y-m-d')) && $estimates_info->status == 'draft') {
    ...
    <div class="alert bg-danger-light hidden-print">
        ...
        <?= lang('estimate_overdue') . ' requisition_details.php' . lang('by') . ' ' . $days_between . ' ' . lang('days') ?>
    </div>
    <?php
}
?>
            ...
            <table class="table items estimate-items-preview" page-break-inside: auto;>
                ...
                <tr>
                    ...
                    if (isset($estimates_info) && $estimates_info->show_quantity_as == 'hours' || isset($hours_quantity)) {
                        ...
                        $qty_heading = lang('qty') . 'requisition_details.php/' . lang('hours');
                    }
                    ?>
                ...
                <tbody>
                <?php
                $invoice_items = $this->requisition_model->ordered_items_by_id($estimates_info->requisition_id);
