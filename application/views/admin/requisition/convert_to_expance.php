                        ...
                        <div class="form-group">
                            ...
                            <div class="col-lg-7">
                                ...
                                if (config_item('increment_invoice_number') == 'FALSE') {
                                    ...
                                    echo $this->requisition_model->generate_transactions_number('Expense');
                                }
                                ?>" name="reference_no">
                            </div>
...
<!--                                --><?php //} ?>
            ...
            <?php
            $saved_items = $this->requisition_model->get_all_items();
            ?>
            <div class="row">
                <div class="col-md-4">
                    <div class="form-group">
                ...
                <div class="table-responsive s_table">
                    <table class="table invoice-items-table items">
                        ...
                        <?php if (isset($estimates_info)) {
                            echo form_hidden('merge_current_invoice', $estimates_info->requisition_id);
                            echo form_hidden('isedit', $estimates_info->requisition_id);
                        }
                        ?>
                        <tr class="main">
                            <td></td>
                        ...
                        <?php if (isset($estimates_info) || isset($add_items)) {
                            ...
                            if (isset($estimates_info)) {
                                $add_items = $this->requisition_model->ordered_items_by_id($estimates_info->requisition_id);
                                $items_indicator = 'items';
                            }
                            foreach ($add_items as $item) {
                                ...
                                $table_row .= form_hidden('' . $items_indicator . '[' . $i . '][items_id]', $item->requisition_items_id);
                                $table_row .= form_hidden('' . $items_indicator . '[' . $i . '][saved_items_id]', $item->saved_items_id);
                                $amount = $item->unit_cost * $item->quantity;
                                $amount = ($amount);
                                // order input
                                $table_row .= '<input type="hidden" class="order" name="' . $items_indicator . '[' . $i . '][order]"><input type="hidden" name="items_id[]" value="' . $item->requisition_items_id . '">';
                                $table_row .= '</td>';
                                ...
                                $table_row .= '<td class="taxrate">' . $this->admin_model->get_taxes_dropdown('' . $items_indicator . '[' . $i . '][taxname][]', $invoice_item_taxes, 'invoice', $item->requisition_items_id, true, $manual) . '</td>';
                                $table_row .= '<td class="amount">' . $amount . '</td>';
                                $table_row .= '<td><a href="#" class="btn-xs btn btn-danger pull-left" onclick="delete_item(this,' . $item->requisition_items_id . '); return false;"><i class="fa fa-trash"></i></a></td>';
                                $table_row .= '</tr>';
                                echo $table_row;
                                $i++;
                            }
