...
<script type="text/javascript">
    ...
    function loadPosItems() {
        ...
        if (get('posItems')) {
            ...
            $.each(posItems, function() {
                ...
                var product_id = data.saved_items_id,
                    item_cost = data.rate,
                    item_qty = data.qty,
                    item_code = data.code,
                ...
                get_taxes_dropdown_template(tax_name, data.taxname).done(function(tax_dropdown) {
                    ...
                    table_row += '<td><textarea  name="items[' + item_key + '][item_desc]" class="form-control item_item_desc RitemDesc" >' + ((data.item_desc) ? data.item_desc.replace(regex, "\n") : '')  + '</textarea></td>';
    ...
    function delete_item(row, itemid) {
        setTimeout(function() {
            ...
            if (posItems.hasOwnProperty(itemid)) {} else {
    ...
    function add_item_to_table(data, itemid, merge_invoice) {
        ...
        get_taxes_dropdown_template(tax_name, data.taxname).done(function(tax_dropdown) {
            ...
            table_row += '<td><textarea  name="items[' + item_key + '][item_desc]" class="form-control item_item_desc" >' + ((data.item_desc) ? data.item_desc.replace(regex, "\n") : '') + '</textarea></td>';
            <?php $invoice_view = config_item('invoice_view');
            if (!empty($invoice_view) && $invoice_view == '2') { ?>
                table_row += '<td><input type="text" name="items[' + item_key + '][hsn_code]" class="form-control" value="' + data.hsn_code + '"></td>';
            <?php } ?>
